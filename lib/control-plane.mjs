import { saveConfigAtomic } from "./config.mjs";
...
function validateBootstrap(payload) {
  ...
  if (!Array.isArray(payload.models)) {
    throw new ControlPlaneError("invalid_bootstrap", "bootstrap.models must be an array");
  }
  if (payload.updates?.npm_package && payload.updates.npm_package !== "9codex") {
    throw new ControlPlaneError("invalid_bootstrap", "bootstrap update package must be 9codex");
  }
  return payload;
}
...
function applyBootstrap(config, bootstrap, accessToken, tokenExpiresIn) {
  ...
  updated.updates = {
    ...
    target_version: bootstrap.updates?.latest_version || updated.updates.target_version,
    minimum_version: bootstrap.updates?.minimum_version || null,
    npm_registry: bootstrap.updates?.npm_registry || updated.updates.npm_registry,
  };
...
export async function syncBootstrap(paths, config, options = {}) {
  ...
  if (response.status === 304) {
    if (refreshed) {
      ...
      saveConfigAtomic(paths, working);
    }
  ...
  saveConfigAtomic(paths, updated);
  return { changed: true, config: updated, bootstrap: payload };
}
