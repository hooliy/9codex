...
export async function uninstallService(paths, options = {}) {
  ...
  const desktopEnvironment = await run(
    "/bin/launchctl",
    ["getenv", "CODEX_CLI_PATH"],
    { capture: true },
  );
  const currentWrapper = typeof desktopEnvironment === "number"
    ? ""
    : desktopEnvironment.stdout.trim();
  const wrapperRoot = `${paths.stateDir}${path.sep}codex-wrapper-`;
  if (currentWrapper.startsWith(wrapperRoot)) {
    await run("/bin/launchctl", ["unsetenv", "CODEX_CLI_PATH"]);
  }
