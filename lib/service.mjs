...
function windowsTaskRegistration(paths, nodePath, cliPath) {
  const daemonCommand = [
    "while ($true) {",
    `  & ${powershellLiteral(nodePath)} ${powershellLiteral(cliPath)} daemon`,
    `  *>> ${powershellLiteral(paths.daemonLog)};`,
  ...
  return [
    ...
    `$actionArgs = ${powershellLiteral(actionArguments)}`,
    `$action = New-ScheduledTaskAction -Execute \"$PSHOME\\powershell.exe\" -Argument $actionArgs -WorkingDirectory ${powershellLiteral(paths.home)}`,
