...
export function validateAuthoritativeModels(models) {
  ...
  for (const row of models) {
    ...
    seen.add(id);
    if (row.protocol === "chat_compat") continue;
    if (![undefined, "responses_native", "responses_compat"].includes(row.protocol)) {
      throw new Error(`Model "${id}" does not support the Responses API required by Codex`);
    }
    ...
    usable.push(structuredClone({ ...row, id }));
  }
