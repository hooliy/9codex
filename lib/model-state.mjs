...
import { isGptModelId, refreshUpstreamModels } from "./models.mjs";

const TRANSACTION_PREFIX = ".model-state-tx-";
const LOCK_FILE = "model-state.lock";
const ROUTING_ALIASES = new Set(["Fast", "Pro", "Image"]);
...
function requireNonEmptyBuiltState(built) {
  if (
    !Array.isArray(built?.models)
    || built.models.length === 0
    || !built.map
    || Object.keys(built.map).length === 0
    || !built.protocols
    || Object.keys(built.protocols).length === 0
  ) {
    throw new Error("Model state must contain at least one usable model");
  }
  return built;
}
...
export function validateAuthoritativeModels(models) {
  ...
  for (const row of models) {
    ...
    seen.add(id);
    if (!Object.hasOwn(row, "context_window")) {
      if (ROUTING_ALIASES.has(id)) continue;
      throw new Error(`Missing context_window for model "${id}"`);
    }
    if (
    ...
    usable.push(structuredClone({ ...row, id, protocol: "responses_native" }));
  }
  if (usable.length === 0) throw new Error("No usable model metadata available");
  return usable;
}

function convergeSelection(config) {
  const available = config.models.available.filter(
    (row) => row?.enabled !== false && typeof row?.id === "string" && row.id.length > 0,
  );
  const ids = new Set(available.map((row) => row.id));
  if (ids.size === 0) throw new Error("Model selection must contain at least one usable model");
  ...
  if (!enabled.includes(config.upstream.default_model)) {
    config.upstream.default_model = config.upstream.default_model === "Fast"
      ? enabled.find(isGptModelId) || enabled[0]
      : enabled[0];
  }
  return config;
}
...
function loadPreparedState(paths, { recover = true } = {}) {
  ...
  const built = requireNonEmptyBuiltState(buildCatalog(persistedConfig));
...
function validatePreparedState(paths, config, options = {}) {
  ...
  expected.models.available = validateAuthoritativeModels(expected.models.available);
  convergeSelection(expected);
  validateConfig(expected);
  if (!isDeepStrictEqual(state.config, expected)) {
...
export async function reconcileModelState(paths, currentConfig, options = {}) {
  ...
  const built = requireNonEmptyBuiltState(buildCatalog(prepared));
  commitModelState(paths, prepared, built);
  return { config: prepared, built };
}

export function repairLocalModelState(paths, currentConfig) {
  const repaired = structuredClone(currentConfig);
  repaired.models.available = validateAuthoritativeModels(repaired.models.available);
  convergeSelection(repaired);
  validateConfig(repaired);
  const built = requireNonEmptyBuiltState(buildCatalog(repaired));
  commitModelState(paths, repaired, built);
  return repaired;
}
