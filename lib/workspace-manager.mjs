...
export class WorkspaceManager {
  ...
  createWorktree(options) {
    ...
    if (fs.existsSync(destination)) {
      const restored = this.restoreWorktree(options);
      const status = workingTreeStatus(destination, true);
      if (!status && startPoint === "HEAD") {
        const head = git(this.repo, ["rev-parse", "HEAD"]);
        if (git(destination, ["rev-parse", "HEAD"]) !== head) {
          git(destination, ["reset", "--hard", head]);
          restored.base_commit = head;
          restored.head = head;
          this.#mutate((state) => {
            if (state.worktrees[String(workItem)]) state.worktrees[String(workItem)].base_commit = head;
          });
        }
      }
      return restored;
