    ...
    requirements: {
      ...
      items: {
        ...
        properties: {
          ...
          impactActions: {
            type: "object",
            additionalProperties: { type: "string", enum: ["rework", "revalidate", "stale", "canceled"] },
          },
          workItems: {
            type: "array",
            minItems: 1,
...
export function normalizeDemandProposal(value, fallbackContent = "") {
  ...
  const requirements = value.requirements.map((entry, index) => {
    ...
    return {
      ...
      impactActions: plainObject(entry.impactActions, `proposal.requirements[${index}].impactActions`),
      workItems,
    };
  });
  return {
