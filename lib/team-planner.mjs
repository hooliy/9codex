import { normalizeDemandProposal } from "./demand-intake.mjs";
...
function instruction(input) {
  return [
    ...
    "The JSON must contain summary, questions, requirements.",
    "Each requirement must contain key, requirementId, title, normalizedRequirement, impactSummary, acceptanceCriteria, impactActions, workItems.",
    "Each workItem must contain key, title, description, priority, writeSet, readSet, resourceLocks, dependencies, acceptanceCriteria.",
    "Each acceptance criterion must contain id and command as a non-empty argv array.",
    "Each impactAction must contain workItemId and action. action must be rework, revalidate, stale, or canceled.",
    "Use null for a new requirement's requirementId. Use arrays for impactActions and all path/dependency fields.",
...
function parsePlannerJson(text) {
  ...
  if (!source) throw new Error("planner returned empty output");
...
export function createTeamPlanner(options = {}) {
  ...
  return async (input) => {
    ...
    try {
      ...
      return normalizeDemandProposal(parsePlannerJson(plannerText(runtime.readEvents(worker))), input.content);
    } finally {
