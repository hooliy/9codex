...
export class HarnessRuntime {
  ...
  inspectWorker(workerOrId) {
    const worker = this.#worker(workerOrId);
    let phase = worker.status === "running" ? "model_waiting" : "idle";
    let activeSubagents = 0;
    for (const event of worker.events) {
      const notification = event.data?.notification;
      if (notification?.method === "subagent.started") activeSubagents += 1;
      if (notification?.method === "subagent.finished") activeSubagents = Math.max(0, activeSubagents - 1);
      const eventType = notification?.params?.event?.type || "";
      if (/^(?:tool|command|process)\/(?:start|started)$/i.test(eventType)) activeSubagents += 1;
      if (/^(?:tool|command|process)\/(?:end|ended|finish|finished)$/i.test(eventType)) {
        activeSubagents = Math.max(0, activeSubagents - 1);
      }
    }
    if (activeSubagents) phase = "tool_running";
    const lastEvent = worker.events.at(-1);
    const inner = lastEvent?.data?.notification?.params?.event;
    const failed = lastEvent?.type === "run.failed" || /error|failed/i.test(inner?.type || "");
    const rawFailure = failed ? {
      code: lastEvent?.data?.code || inner?.data?.code,
      message: lastEvent?.data?.message || inner?.data?.message || inner?.type,
    } : null;
    return {
      phase,
      lastRuntimeEventAt: worker.lastEventAt || null,
      processAlive: !worker.transport.closed,
      failure: rawFailure ? { ...classifyRuntimeFailure(rawFailure), ...rawFailure } : null,
    };
  }
