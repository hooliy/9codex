...
export function validateConfig(config) {
  ...
  config.team.model_call_stall_seconds ??= 900;
  config.team.model_call_start_timeout_seconds ??= 300;
  if (
    !Number.isInteger(config.team.model_call_start_timeout_seconds)
    || config.team.model_call_start_timeout_seconds < 1
  ) {
    throw new Error("team.model_call_start_timeout_seconds must be a positive integer");
  }
  if (!Number.isInteger(config.team.model_call_stall_seconds) || config.team.model_call_stall_seconds < 1) {
    throw new Error("team.model_call_stall_seconds must be a positive integer");
  }
