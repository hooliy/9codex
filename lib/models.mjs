...
function contextWindowField(row) {
  ...
  return positiveIntegerValue(row, "context_window", declared.value);
}
...
export async function refreshUpstreamModels(config, options = {}) {
  ...
  const rows = Array.isArray(payload?.data)
    ? payload.data.filter((row) => (
        ...
        && row.id.length > 0
        && [undefined, "responses_native", "responses_compat"].includes(row.protocol)
      ))
    : [];
  if (rows.length === 0) throw new Error("Upstream returned no Codex-compatible Responses models");
  const seen = new Set();
  return rows.filter((row) => {
    ...
    return {
      ...
      protocol: ["responses_native", "responses_compat"].includes(row.protocol)
        ? row.protocol
        : "responses_native",
