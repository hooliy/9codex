...
// ponytail: Unknown upstream capacity is capped at 128k; remove this fallback once /models requires context_window.
export const CONSERVATIVE_CONTEXT_WINDOW = 128_000;
...
function positiveIntegerValue(row, field, value) {
...
function declaredLimit(row, field, capabilityField) {
  if (Object.hasOwn(row, field)) return { present: true, value: row[field] };
  if (Object.hasOwn(row.capabilities || {}, capabilityField)) {
    return { present: true, value: row.capabilities[capabilityField] };
  }
  return { present: false, value: undefined };
}

function contextWindowField(row) {
  const declared = declaredLimit(row, "context_window", "contextWindow");
  return declared.present
    ? positiveIntegerValue(row, "context_window", declared.value)
    : CONSERVATIVE_CONTEXT_WINDOW;
}

function maxOutputTokensField(row) {
  const declared = declaredLimit(row, "max_output_tokens", "maxOutput");
  if (!declared.present) return undefined;
...
export async function refreshUpstreamModels(config, options = {}) {
  ...
  return rows.filter((row) => {
    ...
    const maxOutputTokens = maxOutputTokensField(row);
    return {
      id: row.id,
      display_name: row.display_name || row.id,
      enabled: row.enabled !== false,
      protocol: ["responses_native", "responses_compat", "chat_compat"].includes(row.protocol)
        ? row.protocol
        : "chat_compat",
