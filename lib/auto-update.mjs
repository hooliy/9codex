...
export async function getGatewayActivity(paths, options = {}) {
  try {
    const config = (options.loadConfig || loadConfig)(paths);
    const response = await (options.fetchImpl || fetch)(
      `http://${config.local.host}:${config.local.port}/healthz`,
      { signal: AbortSignal.timeout(options.timeoutMs || 1000) },
    );
    if (!response.ok) return { state: "unknown" };
    const health = await response.json();
    if (!health?.ready || !Number.isInteger(health.active_requests)) return { state: "unknown" };
    return {
      state: health.active_requests === 0 ? "idle" : "busy",
...
export async function waitForGatewayIdle(paths, options = {}) {
  const wait = options.wait || ((milliseconds) =>
    new Promise((resolve) => setTimeout(resolve, milliseconds)));
  while (true) {
    const activity = await (options.getActivity || getGatewayActivity)(paths, options);
