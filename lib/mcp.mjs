...
async function imageContent(payload, format, fetchImpl) {
  ...
  for (const row of rows) {
    ...
    if (typeof row?.url === "string" && row.url.length > 0) {
      const downloaded = await fetchImpl(row.url);
...
export async function handleMcpRequest(config, request, options = {}) {
  ...
  if (
    ...
    return response(request.id, request.method === "resources/templates/list"
      ? { resourceTemplates: [] }
      : request.method === "prompts/list"
        ? { prompts: [] }
        : { resources: [] });
  }
  if (request.method === "tools/list") return response(request.id, { tools: [IMAGE_TOOL] });
  if (request.method === "tools/call") {
    ...
    try {
      ...
      return response(request.id, {
        content: [{ type: "text", text: `9codex image generation failed: ${error.message}` }],
        isError: true,
      });
    }
  }
