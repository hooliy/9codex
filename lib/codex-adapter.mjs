...
export class CodexAdapter {
  ...
  async createThread(instruction, options = {}) {
    ...
    const worker = this.createWorker(instruction, {
      ...
      onEvent: (event, activeWorker) => {
        ...
        this.interruptWorker(activeWorker);
        const timer = this.setTimeout(() => this.terminateWorker(activeWorker), 5000);
        timer?.unref?.();
      },
    });
    try {
  ...
  inspectWorker(workerOrId) {
    const worker = this.#worker(workerOrId);
    let phase = worker.currentRun?.settled ? "idle" : "starting";
    ...
    for (const event of worker.events) {
      if (["turn.started", "thread.started"].includes(event.type)) phase = "model_waiting";
      if (event.type === "item.started") {
        phase = toolTypes.has(event.item?.type) ? "tool_running" : "model_waiting";
      }
      if (event.type === "item.completed") phase = "model_waiting";
      if (["turn.completed", "turn.failed", "adapter.process_exit"].includes(event.type)) phase = "idle";
    }
    let retrySignalCount = 0;
    for (let index = worker.events.length - 1; index >= 0; index -= 1) {
      const event = worker.events[index];
      const text = `${event?.message || ""} ${event?.text || ""} ${event?.error?.message || ""}`;
      if (/retry|rate.?limit|too many requests|temporar|unavailable/i.test(text)) retrySignalCount += 1;
      else if (text.trim()) break;
    }
    return {
      phase,
      lastRuntimeEventAt: worker.lastEventAt || null,
      processAlive: Boolean(worker.currentRun && !worker.currentRun.settled),
      lastEvent: worker.events.at(-1) || null,
      retrySignalCount,
    };
  }
