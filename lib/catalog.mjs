...
function catalogRow(row, slug, displayName, serviceTiers, priority) {
  ...
  return {
    ...
    auto_compact_token_limit: Math.max(
      1,
      row.context_window - Math.ceil(row.context_window / 10),
    ),
    truncation_policy: {
      mode: "tokens",
...
export function buildCatalog(config) {
  ...
  for (const row of source) {
    ...
    models.push(catalogRow(
      row,
      slug,
      `${MODEL_DISPLAY_PADDING}${row.display_name || row.id}`,
      serviceTiers,
      models.length + 1,
    ));
  }
