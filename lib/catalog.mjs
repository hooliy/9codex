...
function fastModelSlug(modelId) {
  return `9codex-fast/${Buffer.from(modelId).toString("base64url")}`;
}
...
export function buildCatalog(config) {
  ...
  for (const row of source) {
    ...
    if (serviceTiers.length > 0) {
      const fastSlug = fastModelSlug(row.id);
      map[fastSlug] = row.id;
      forcedServiceTiers[fastSlug] = "priority";
      models.push(catalogRow(
        row,
        fastSlug,
        `${row.display_name || row.id} · 快速模式`,
        serviceTiers,
        models.length + 1,
      ));
    }
