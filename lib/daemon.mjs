...
import { enableModelPicker } from "./codex-ui.mjs";
...
export async function maintainCodexModelPicker(state, context = {}) {
  if (state.restarting) return "busy";
  if (await context.maintenanceActive?.()) return "busy";
  const processes = await (context.listProcesses || listCodexProcesses)();
  ...
  if (context.loadSession) {
    try {
      ...
      if (state.port == null && Number.isInteger(session?.debug_port)) {
        state.port = session.debug_port;
      }
    } catch {}
  }
  const savedProcessIds = Array.isArray(session?.process_ids)
    ? session.process_ids.filter(Number.isInteger)
    : [];
  const sessionWasReplaced = savedProcessIds.length > 0
    && !processes.some((process) => savedProcessIds.includes(process.pid));
  if (state.port != null) {
    try {
      await (context.enableModelPicker || enableModelPicker)({ port: state.port });
      state.failures = 0;
      return "healthy";
    } catch {
      state.port = null;
  ...
  if (sessionWasReplaced) state.failures = 2;
  state.failures += 1;
  if (state.failures < 3) return "waiting";
  state.restarting = true;
  try {
    const result = await (context.restartCodex || restartCodex)();
    state.port = result.debug_port;
    state.failures = 0;
    return "restarted";
  } finally {
...
export function desktopMaintenanceActive(file, now = Date.now()) {
  try {
    const marker = JSON.parse(fs.readFileSync(file, "utf8"));
    if (Number(marker.expires_at) > now) return true;
  } catch {}
  try {
    fs.unlinkSync(file);
  } catch {}
    ...
    try {
      ...
      await consumeEventStream(response, async (command) => {
        ...
        await dispatchRemoteCommand(command, {
          ...
          writeCatalog: async (updated) => writeCatalog(paths, buildCatalog(updated)),
...
export async function runDaemon(paths, options = {}) {
  ...
  heartbeat.unref();
  const desktopState = { port: null, failures: 0, restarting: false };
  const desktopWatch = setInterval(() => {
    void maintainCodexModelPicker(desktopState, {
      ...options,
      maintenanceActive: () => desktopMaintenanceActive(paths.desktopMaintenance),
      loadSession: options.loadSession || (() => JSON.parse(fs.readFileSync(paths.desktopSession, "utf8"))),
      restartCodex: options.restartCodex || (() => restartCodex({
        ...options,
