...
export async function runDaemon(paths, options = {}) {
  ...
  const config = loadConfig(paths);
  (options.validateModelState || validateModelState)(paths, config);
  let teamRuntime = null;
  const gatewayOptions = { ...options };
  if (config.team?.enabled !== false) {
    const start = options.startTeamRuntime
      || (await import("./team-runtime.mjs")).startTeamRuntime;
    teamRuntime = await start(paths, config, {
      codexCommand: options.codexCommand,
      defaultWorkspace: options.defaultWorkspace || paths.home,
      onError: options.onError,
    });
    gatewayOptions.onTeamEvent = (event) => teamRuntime.observeTeamEvent(event);
  }
  const server = await startGatewayServer(paths, config, gatewayOptions);
  ...
  return new Promise((resolve) => {
    const close = () => {
      ...
      server.close(() => {
        void Promise.resolve(teamRuntime?.close()).finally(resolve);
      });
    };
