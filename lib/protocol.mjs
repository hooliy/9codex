...
function normalizeResponsesInput(input) {
  ...
  for (const item of input) {
    ...
    if (item?.type === "function_call_output") {
      ...
      if (output === null) {
        const callIndex = normalized.findIndex((candidate) => (
          candidate?.type === "function_call"
          && (candidate.call_id === item.call_id || candidate.id === item.call_id)
        ));
        if (callIndex !== -1) normalized.splice(callIndex, 1);
        pendingToolCalls.delete(item.call_id);
        continue;
      }
      normalized.push({ ...item, output });
      pendingToolCalls.delete(item.call_id);
      continue;
    }
