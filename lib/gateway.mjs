...
function goalCallKey(payload) {
  return payload?.item_id || payload?.item?.id || (
    Number.isInteger(payload?.output_index) ? `index:${payload.output_index}` : null
  );
}

function restoreNativeSseBlock(block, goalCalls) {
  return block.split(/\r?\n/).map((line) => {
    if (!line.startsWith("data:") || line.trim() === "data: [DONE]") return line;
    try {
      const payload = JSON.parse(line.slice(5).trimStart());
      if (
        ["function_call", "custom_tool_call"].includes(payload?.item?.type)
        && payload.item.name === "create_goal"
      ) {
        const key = goalCallKey(payload);
        if (key) goalCalls.add(key);
      }
      if (
        payload?.type?.startsWith?.("response.function_call_arguments.")
        && goalCalls.has(goalCallKey(payload))
      ) {
        if (Object.hasOwn(payload, "delta")) payload.delta = "";
  ...
  try {
    for await (const chunk of response.body || []) {
      ...
      while ((boundary = buffer.search(/\r?\n\r?\n/)) !== -1) {
        ...
        res.write(`${restoreNativeSseBlock(block, goalCalls)}${separator}`);
      }
    }
  } catch {}
  if (buffer) res.write(restoreNativeSseBlock(buffer, goalCalls));
  try {
    res.end();
  } catch {}
}
...
async function fetchNative(req, config, route, body, fetchImpl) {
  ...
  let outbound = flattenNineCodexRequest(repaired);
  const attempts = new Map();
  const limit = (
    (Array.isArray(outbound.input) ? outbound.input.length * 2 : 0)
    + (Array.isArray(outbound.tools) ? outbound.tools.length * 2 : 0)
    + 16
  );
  for (let attempt = 0; ; attempt += 1) {
    const response = await fetchImpl(
      new URL(req.url, `${config.upstream.base_url.replace(/\/$/, "")}/`),
      {
        method: req.method,
    ...
    const invalid = await invalidRequest(response);
    if (!invalid) return response;
    const fingerprint = `${invalid.code || ""}:${invalid.param}`;
    const count = (attempts.get(fingerprint) || 0) + 1;
    attempts.set(fingerprint, count);
    const next = repairInvalidRequest(outbound, invalid, count);
...
function embeddedError(error) {
  if (!error || typeof error !== "object") return null;
  if (typeof error.param === "string") return error;
  if (typeof error.message !== "string") return null;
  const start = error.message.indexOf("{");
  if (start !== -1) {
    for (
      let end = error.message.lastIndexOf("}");
      end > start;
      end = error.message.lastIndexOf("}", end - 1)
    ) {
      try {
        const parsed = JSON.parse(error.message.slice(start, end + 1));
        const nested = embeddedError(parsed?.error);
        if (nested) return nested;
      } catch {}
    }
  }
  const param = error.message.match(/\b(input|tools)(?:\[\d+\]|\.[A-Za-z_]\w*)+/)?.[0];
  return param ? { ...error, param } : null;
}

async function invalidRequest(response) {
  ...
  const error = embeddedError(payload?.error);
  if (!error?.param) return null;
  const schemaError = (
    error.type === "invalid_request_error"
    || String(error.code || "").includes("array_")
    || /invalid|expected|maximum length|unknown field|extra field/i.test(error.message || "")
  );
  return schemaError ? error : null;
}

function requestPath(param) {
  const parts = [];
  for (const match of param.matchAll(/(?:^|\.)([A-Za-z_]\w*)|\[(\d+)\]/g)) {
    parts.push(match[1] ?? Number(match[2]));
  }
  return parts;
}

function repairInvalidRequest(request, error, repeat) {
  const missingOutput = error.message?.match(
    /No tool output found for function call\s+([A-Za-z0-9_-]+)\.?/i,
  );
  if (missingOutput && Array.isArray(request.input)) {
    const callIdPrefix = missingOutput[1];
    const result = structuredClone(request);
    const matchingCallIds = new Set(result.input.flatMap((item) => {
      const callId = item?.call_id || (
        item?.type === "function_call" ? item.id : null
      );
      return typeof callId === "string" && callId.startsWith(callIdPrefix)
        ? [callId]
        : [];
    }));
    if (matchingCallIds.size === 0) return null;
    const repaired = result.input.filter((item) => (
      !(
        item?.type === "function_call"
        && matchingCallIds.has(item.call_id || item.id)
      )
      && !(
        item?.type === "function_call_output"
        && matchingCallIds.has(item.call_id)
      )
    ));
    if (repaired.length === result.input.length) return null;
    result.input = repaired;
    return result;
  }

  const param = error.param;
  const path = requestPath(param);
  if (path.length === 0 || !["input", "tools"].includes(path[0])) return null;
  const result = structuredClone(request);
  const collection = result[path[0]];
  const index = path[1];
  if (!Array.isArray(collection) || !Number.isInteger(index) || index >= collection.length) return null;
  if (repeat > 1) {
  ...
  for (const part of path.slice(0, -1)) {
    if (!parent || typeof parent !== "object" || !(part in parent)) return null;
    parent = parent[part];
  }
  const field = path.at(-1);
  if (!parent || typeof parent !== "object" || !(field in parent)) return null;

  if (path[0] === "input") {
    if (path.length === 3 && field === "content" && repeat === 1) {
      parent[field] = [];
      return result;
    }
    if (path.length > 2 && repeat === 1) {
      delete parent[field];
      return result;
    }
    return null;
