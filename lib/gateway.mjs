...
export function routeForModel(paths, requested, defaultModel) {
  ...
  if (!upstreamModel && typeof requested === "string") {
    const separator = requested.indexOf("/");
    if (separator !== -1 && separator < requested.length - 1) {
      const unprefixed = requested.slice(separator + 1);
      const rebound = `9codex/${unprefixed.replaceAll("/", "-")}`;
      upstreamModel = routing.aliases[rebound];
    }
  }
...
export function createGateway(config, paths, options = {}) {
  ...
  const server = http.createServer((req, res) => {
    void (async () => {
      ...
      const route = routeForModel(paths, body.model, config.upstream.default_model);
