...
function routingFromModelMap(payload) {
  return {
    ...
    serviceTiers: payload.forced_service_tiers || {},
  };
}
...
function routeFromRouting(routing, requested, defaultModel) {
  ...
  return {
    upstreamModel,
    serviceTier: routing.serviceTiers[requested],
...
async function fetchNative(req, config, route, body, fetchImpl) {
  ...
  const repaired = route.protocol === "responses_compat"
    ...
    : normalized;
  const serviceTier = route.serviceTier ?? body.service_tier;
  if (isGptModelId(route.upstreamModel) && serviceTier !== undefined) {
    repaired.service_tier = serviceTier;
  } else {
...
async function proxyChat(req, res, config, route, body, fetchImpl) {
  ...
  if (route.serviceTier !== undefined) routedBody.service_tier = route.serviceTier;
  if (!isGptModelId(route.upstreamModel)) delete routedBody.service_tier;
...
async function proxyAuto(req, res, config, route, body, fetchImpl) {
  ...
  await proxyChat(req, res, config, route, body, fetchImpl);
}
...
function reportTeamEventError(options, error) {
  try {
...
export function createGateway(config, paths, options = {}) {
  ...
  const emitTeamEvent = (event) => {
    if (typeof options.onTeamEvent !== "function") return;
    try {
      Promise.resolve(options.onTeamEvent(event)).catch((error) => reportTeamEventError(options, error));
    } catch (error) {
      reportTeamEventError(options, error);
    }
  };
  const server = http.createServer((req, res) => {
    void (async () => {
      ...
      if (req.method === "POST" && url.pathname === "/v1/responses") {
        const identity = {
          threadId: headerValue(req, "thread-id"),
          sessionId: headerValue(req, "session-id"),
          requestId: headerValue(req, "x-client-request-id") || crypto.randomUUID(),
          subagent: headerValue(req, "x-openai-subagent") !== null,
          path: url.pathname,
          method: req.method,
        };
        const publish = (type, status = null) => emitTeamEvent({
          type,
          ...identity,
          status,
          timestamp: new Date().toISOString(),
        });
        if (identity.threadId && !observedThreads.has(identity.threadId)) {
          observedThreads.add(identity.threadId);
          publish("thread_observed");
        }
        let completed = false;
        const complete = () => {
          if (completed) return;
          completed = true;
          publish("request_completed", res.statusCode);
        };
        res.once("finish", complete);
        res.once("close", complete);
        publish("request_started");
      }
      const modelRoute = (
      ...
      if (route.protocol === "chat_compat") {
        ...
        await proxyAuto(req, res, activeConfig, route, body, fetchImpl);
