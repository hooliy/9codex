    ...
    const promise = (async () => {
      ...
      const watchdog = setInterval(() => {
        ...
        if (worker && Date.now() - worker.lastEventAt > timeoutMs * 10) {
          selectedRuntime.interruptWorker(worker);
        }
      ...
      try {
        ...
        if (!result.ok) {
          await this.orchestrator.failWorker({
            ...
            failure: {
              ...
              signal: result.signal,
              events: events
                .filter((event) => event.type === "run.output" && event.data?.text)
        ...
        try {
          await this.orchestrator.failWorker({
            ...
            runId: assignment.runId,
            failure: {
              type: error?.code === "RUNTIME_TIMEOUT" ? "timeout" : "worker_process",
              message: error.message,
              timedOut: error?.code === "RUNTIME_TIMEOUT",
            },
          });
        } catch (reportError) {
...
export async function startTeamRuntime(paths, config, options = {}) {
  ...
  const harnessRuntime = options.harnessRuntime || createHarnessRuntime({
