...
function finalMessage(events) {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    ...
    const text = event?.item?.text
      || event?.item?.content?.map?.((part) => part?.text).filter(Boolean).join("\n")
      || event?.item?.output
...
function workerActivity(event) {
  ...
  return {
    ...
    command: shortText(item.command || event?.command),
    output: shortText(
    ...
    files: item.files || item.changes || event?.files || null,
  };
}
    ...
    try {
      ...
      for (const action of result.actions || []) {
        if (action.type === "confirm") {
          ...
          if (!group || group.status !== "awaiting_confirmation") continue;
          await this.orchestrator.confirmDemand({
            eventKey: action.eventKey,
            approved: true,
            approvalSourceMessageId: `task-center:${crypto.randomUUID()}`,
          });
  ...
  #supervise(assignment) {
    ...
    const promise = (async () => {
      ...
      const workerTimeoutMs = Math.max(
        60_000,
        Number(this.config.team.worker_timeout_seconds || 300) * 1000,
      );
      const watchdog = setInterval(() => {
        ...
        if (worker && Date.now() - worker.lastEventAt > timeoutMs * 10) {
      ...
      try {
        ...
        try {
          await this.orchestrator.failWorker({
            ...
            runId: assignment.runId,
            failure: {
              type: error?.code === "CODEX_WORKER_TIMEOUT" ? "timeout" : "worker_process",
              message: error.message,
              timedOut: error?.code === "CODEX_WORKER_TIMEOUT",
            },
          });
        } catch (reportError) {
...
export async function startTeamRuntime(paths, config, options = {}) {
  ...
  const verificationEnv = {
    ...workerEnv,
    PATH: [path.dirname(process.execPath), workerEnv.PATH].filter(Boolean).join(path.delimiter),
  };
  ...
  const recordWorkerEvent = ({ taskGroupId, workItemId, workerSessionId, runId, event }) => {
