...
export function buildTaskCenterBridgeScript(options) {
  ...
  return `(() => {
    ...
    const rightSidebarFor = (entry) => {
      let node = entry;
      let fallback = null;
      while (node && node !== document.body) {
        const bounds = node.getBoundingClientRect();
        const rightAligned = bounds.right >= window.innerWidth - 2;
        const sidebarSized = bounds.width >= 280
          && bounds.width <= Math.min(760, window.innerWidth * 0.48)
          && bounds.height >= window.innerHeight * 0.65;
        if (rightAligned && sidebarSized) {
          fallback = node;
          if (node.classList.contains("isolate")) return node;
        }
        node = node.parentElement;
      }
      return fallback;
    };
    const placePanel = (sidebar) => {
    ...
    const open = () => {
      state.open = true;
      const entry = document.querySelector("#ninecodex-session-tasks-entry");
      if (!placePanel(rightSidebarFor(entry))) {
        state.open = false;
        return false;
      }
    ...
    const ensureMenuEntry = () => {
      ...
      if (existing?.isConnected) {
        if (state.open) placePanel(rightSidebarFor(existing));
        return true;
      }
    ...
    return { applied: true, entry, open: false, selectedGroupId: state.selectedGroupId, tasks: state.payload.length, actions: [] };
  })()`;
}
