...
export function enrichWorkItems(snapshot = {}) {
  const reasonLabels = {
    ...
    worker_process_failed: "执行进程失败，等待手动重试",
    model_call_recovery: "模型调用异常，正在自动修复",
    model_call_recovery_exhausted: "同类模型调用异常已耗尽自动修复",
    upstream_stream_stalled: "上游模型流已停滞",
    upstream_first_event_timeout: "模型调用未产生首个 Runtime 事件",
    upstream_authentication_failed: "上游模型认证失败",
    upstream_rate_limited: "上游模型限流，正在按退避策略修复",
    upstream_transient_failure: "上游模型暂时不可用",
    upstream_request_timeout: "上游模型请求超时",
    upstream_quota_exhausted: "上游模型额度已耗尽",
    model_unavailable: "当前模型不可用",
    runtime_protocol_failure: "Runtime 协议异常",
    user_stopped_work_item: "用户已停止当前尝试",
    user_retry_work_item: "用户已手动重试",
  ...
  return items.map((item) => {
    ...
    const itemRuns = runs.filter((row) => row.work_item_id === item.id);
    ...
    const runtimeEvents = events
      .filter((row) => row.event_type === "worker.runtime_activity" && row.payload?.workItemId === item.id)
      .sort((left, right) => String(left.created_at).localeCompare(String(right.created_at)));
    const recoveryEvent = [...events].reverse().find((row) => (
      row.event_type === "project_manager.replanned"
      && row.payload?.workItemId === item.id
      && ["model_call_recovery", "model_call_recovery_exhausted"].includes(row.payload?.reason)
    ));
    ...
    const latestFailure = latest(snapshot.evidence || [], (row) => (
      row.work_item_id === item.id && ["worker_process", "model_call"].includes(row.type)
    ));
    ...
    else if (["failed", "blocked"].includes(item.status)) [waitingReason, nextAction] = [reason || "执行或验收失败", "检查失败原因后手动重试"];
    ...
    return {
      ...
      last_output_at: currentSessionOutputs.at(-1)?.created_at || null,
      last_runtime_event_at: runtimeEvents.at(-1)?.created_at || null,
      last_progress_at: currentSessionOutputs.at(-1)?.created_at || checkpoint?.created_at || null,
      model_call_phase: runtimeEvents.at(-1)?.payload?.phase || null,
      recovery: recoveryEvent ? {
        at: recoveryEvent.created_at,
        attempt: recoveryEvent.payload?.recoveryAttempt || null,
        action: recoveryEvent.payload?.recoveryAction || null,
      } : null,
      running_since: run?.status === "running" ? run.started_at || run.created_at : null,
      attempt_count: itemRuns.filter((row) => row.role === "worker").length,
      failed_attempt_count: itemRuns.filter((row) => row.role === "worker" && row.status === "failed").length,
      interrupted_attempt_count: itemRuns.filter((row) => row.role === "worker" && row.status === "interrupted").length,
      last_failure: latestFailure ? {
        at: latestFailure.created_at,
        source: latestFailure.source,
        exit_code: latestFailure.exit_code,
        metadata: parseJson(latestFailure.metadata, latestFailure.metadata || {}),
      } : null,
...
export function buildTaskCenterBridgeScript(options) {
  ...
  return `(() => {
    ...
    style.textContent = \`
      ...
      .nine-st-drawer{position:absolute;z-index:2;inset:0 0 0 auto;display:none;width:min(440px,100%);grid-template-rows:auto 1fr auto;border-left:1px solid var(--color-border,rgba(255,255,255,.09));background:var(--color-background-surface,var(--color-background-panel,#181818));box-shadow:-12px 0 36px rgba(0,0,0,.24)}.nine-st-drawer[data-open=true]{display:grid}.nine-st-drawer-head{display:flex;align-items:flex-start;gap:10px;padding:14px;border-bottom:1px solid var(--color-border,rgba(255,255,255,.09))}.nine-st-drawer-title{min-width:0;flex:1;font-weight:650}.nine-st-drawer-body{overflow:auto;padding:14px}.nine-st-detail-row{padding:8px 0;border-bottom:1px solid var(--color-border,rgba(255,255,255,.07));font-size:12px;overflow-wrap:anywhere}.nine-st-detail-label{display:block;margin-bottom:2px;color:var(--color-text-foreground-tertiary,var(--color-text-secondary,rgba(255,255,255,.55)));font-size:10px}.nine-st-history{margin-top:12px}.nine-st-drawer-actions{display:flex;flex-wrap:wrap;gap:8px;padding:12px;border-top:1px solid var(--color-border,rgba(255,255,255,.09))}.nine-st-drawer-actions button{padding:8px 10px;border:1px solid var(--color-border,rgba(255,255,255,.09));border-radius:8px;background:transparent;color:inherit;cursor:pointer}.nine-st-drawer-actions [hidden]{display:none}.nine-st-danger{color:var(--color-text-danger,light-dark(#b42318,#ffb4ab))!important;border-color:color-mix(in srgb,light-dark(#b42318,#ffb4ab) 55%,transparent)!important}
    ...
    const openWorkItem = (item, trigger) => {
      ...
      detailRow(body, "尝试次数", item.attempt_count);
      detailRow(body, "失败 / 中断", Number(item.failed_attempt_count || 0) + " / " + Number(item.interrupted_attempt_count || 0));
      detailRow(body, "开始时间", item.running_since);
      detailRow(body, "监督心跳", item.last_heartbeat_at);
      detailRow(body, "Runtime 活动", item.last_runtime_event_at);
      detailRow(body, "有效进展", item.last_progress_at);
      detailRow(body, "调用阶段", item.model_call_phase);
      detailRow(body, "自动修复", item.recovery);
      detailRow(body, "最后输出", item.last_output_at);
      detailRow(body, "最后失败", item.last_failure);
      ...
      drawer.querySelector("[data-stop-work]").hidden = !["assigned", "running"].includes(item.actual_status || item.status);
      drawer.querySelector("[data-retry-work]").hidden = currentGroup()?.status === "paused" || !["blocked", "failed"].includes(item.actual_status || item.status);
    ...
    const enqueueWorkAction = type => {
      const group = currentGroup();
      const item = group?.work_items?.find(row => row.id === state.selectedWorkItemId);
      if (!item) return;
      enqueueAction({ type, taskGroupId: item.task_group_id, workItemId: item.id });
      closeDrawer();
    };
    drawer.querySelector("[data-stop-work]").addEventListener("click", () => enqueueWorkAction("stop_work_item"));
    drawer.querySelector("[data-retry-work]").addEventListener("click", () => enqueueWorkAction("retry_work_item"));
    drawer.querySelector("[data-delete-work]").addEventListener("click", () => {
      ...
      enqueueAction({ type: "delete_work_item", taskGroupId: item.task_group_id, workItemId: state.selectedWorkItemId });
      closeDrawer();
    });
