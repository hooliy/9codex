...
export function buildTaskCenterPayload(store, options = {}) {
  ...
  const counts = Object.fromEntries([
    ...
    ["done", workItems.filter((item) => ["passed", "closed", "canceled", "stale"].includes(item.status)).length],
...
export function buildTaskCenterBridgeScript(options) {
  ...
  return `(() => {
    ...
    const render = () => {
      ...
      for (const [title, statuses] of lanes) {
        ...
        for (const item of items) {
          ...
          card.addEventListener("click", () => openWorkItem(item));
          list.append(card);
        }
      ...
      clear.addEventListener("click", () => {
        ...
        state.actions.push({ type: "clear_all" });
      });
      toolbar.append(browser, clear);
      body.append(toolbar);
    };

    const placePanel = (aside) => {
    ...
    const open = () => {
      state.open = true;
      const entry = document.querySelector("#ninecodex-task-center-button")
        || document.querySelector("#ninecodex-session-tasks-entry");
      placePanel(entry?.closest("nav") || entry?.closest("aside"));
    ...
    const ensurePrimaryEntry = () => {
      const existing = document.querySelector("#ninecodex-task-center-button");
      if (existing?.isConnected) {
        if (state.open) placePanel(existing.closest("nav") || existing.closest("aside"));
        return true;
      }
      const plugin = [...document.querySelectorAll("button")]
        .find(button => button.textContent.trim() === "插件");
    ...
    window.__ninecodexTaskCenterBridge = {
      ...
      sync() {
        return { ...this.status(), actions: state.actions.splice(0) };
      },
      open,
      close
    };
    return { applied: true, button: entry, entry: Boolean(document.querySelector("#ninecodex-session-tasks-entry")), open: false, selectedGroupId: state.selectedGroupId, tasks: state.payload.length, actions: [] };
  })()`;
}
