...
const DEFAULT_MIGRATIONS = [{ version: 1, up: (db) => db.exec(MIGRATION_1) }];
...
export class TeamStore {
    ...
    return this.transaction(() => {
      ...
      this.db.prepare(`
        INSERT INTO task_groups(
          id, origin_thread_id, title, status, workspace, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(
        recordId,
    ...
    return this.db.prepare(`
      SELECT
        ...
        (SELECT COUNT(*) FROM worker_sessions ws
          WHERE ws.task_group_id = tg.id AND ws.status IN ('creating','running','waiting')) AS running_workers,
  ...
  attachWorkerConversation(workerSessionId, threadId, input = {}) {
    ...
    return this.transaction(() => {
      ...
      this.db.prepare(
        "UPDATE worker_sessions SET codex_thread_id = ?, updated_at = ?, version = version + 1 WHERE id = ?",
      ).run(threadId, nowIso(this.now), workerSessionId);
      ...
      this.#emit(session.task_group_id, "worker_session", workerSessionId, "worker_session.thread_attached", updated.version, {
        threadId,
        actor: input.actor || "orchestrator",
    ...
    return this.transaction(() => {
      ...
      this.db.prepare(`
        INSERT INTO worker_sessions(
          id, task_group_id, work_item_id, codex_thread_id, role, status,
      ...
      this.#emit(input.taskGroupId, "worker_session", recordId, "worker_session.created", 0, {
