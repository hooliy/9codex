...
import { normalizeDemandInput, normalizeDemandProposal } from "./demand-intake.mjs";
...
function isApprovalText(value) {
  ...
  if (SHORT_APPROVAL.test(text)) return true;
  return text.length <= 120
    && /^(?:approve|approved|确认|同意|批准|可以)(?=[\s\S]*(?:执行|开始|任务组|以上|上述|方案|范围))[\s\S]+[。！!，,\s]*$/i.test(text);
}
...
function defaultClassifier({ content, hasRequirements, pendingConfirmation }) {
...
function defaultPlanner({ content, classification, currentRequirement }) {
  ...
  return {
    ...
    requirements: [{
      key: "requirement-1",
      requirementId: currentRequirement?.id || null,
...
export class TaskOrchestrator {
  constructor(options = {}) {
    ...
    this.publishThread = options.publishThread || (() => {});
  ...
  async ingestDemand(input) {
    ...
    const normalized = normalizeDemandInput(input);
    const { sourceMessageId, content } = normalized;
    ...
    const operation = this.#ingestDemand({ ...normalized, threadId, workspace, eventKey });
  ...
  async #ingestDemand(input) {
    ...
    const pendingConfirmation = this.#pendingConfirmation(group.id);
    const rawClassification = await this.classifier({
      content,
      taskGroup: group,
      hasRequirements,
      pendingConfirmation,
    ...
    let classification = typeof rawClassification === "string"
    ...
    if (pendingConfirmation && isApprovalText(content)) {
      classification = {
        type: "approval",
        confidence: 0.99,
        highImpact: false,
    ...
    const requiresConfirmation = classification.requiresConfirmation ?? (
      MUTATING_DEMAND_TYPES.has(classification.type)
      && !classification.executeImmediately
    );
    if (requiresConfirmation) {
      const preview = await this.#planDemand({
        group,
        content,
        classification,
        suppliedProposal: input.proposal,
      });
      this.#setGroupStatus(group.id, "awaiting_confirmation", "demand_proposal_waiting_confirmation");
      result = {
        eventKey,
        taskGroupId: group.id,
        ...
        proposedRequirements: preview.requirements,
      };
      ...
      result = await this.#processDemand({
        group,
        sourceMessageId,
        content,
        classification,
  ...
  async confirmDemand(input) {
    ...
    if (!["awaiting_confirmation", "collecting"].includes(group.status)) {
      throw new Error(`TaskGroup cannot confirm demand while ${group.status}: ${group.id}`);
    }
    ...
    const confirmedEvent = this.store.confirmDemandEvent(eventKey, {
      actor: "user",
      source: input.approvalSourceMessageId || event.source_message_id,
    });
    ...
    result.confirmedAt = confirmedEvent.confirmed_at;
    if (input.approvalSourceMessageId) result.approvalSourceMessageId = input.approvalSourceMessageId;
    this.#finishDemand(eventKey, result);
    return result;
  }

  async #planDemand({ group, content, classification, suppliedProposal }) {
    if (suppliedProposal) return normalizeDemandProposal(suppliedProposal, content);
    const existingRequirements = this.store.db.prepare(`
      SELECT r.id, r.title, r.status, rr.id revision_id, rr.revision,
             rr.normalized_requirement, rr.acceptance_criteria
      FROM requirements r
      LEFT JOIN requirement_revisions rr
        ON rr.requirement_id = r.id AND rr.status = 'active'
      WHERE r.task_group_id = ? AND r.status <> 'canceled'
      ORDER BY r.created_at, r.id
    `).all(group.id);
    const currentRequirement = classification.requirementId
      ? existingRequirements.find((row) => row.id === classification.requirementId)
      : existingRequirements.at(-1);
    ...
    return normalizeDemandProposal(plan, content);
  }

  async #processDemand({ group, sourceMessageId, content, classification, eventKey, source, proposal }) {
    ...
    const existingRevisions = this.store.db.prepare(`
      SELECT rr.* FROM requirement_revisions rr
      JOIN requirements requirement ON requirement.id = rr.requirement_id
      LEFT JOIN demand_events de ON de.id = rr.source_event_id
      WHERE requirement.task_group_id = ?
        AND (de.event_key = ? OR (rr.source_event_id IS NULL AND rr.source_message_id = ?))
      ORDER BY rr.created_at, rr.id
    `).all(group.id, eventKey, sourceMessageId);
    if (existingRevisions.length) {
      const revisionIds = new Set(existingRevisions.map((row) => row.id));
      const existingWorkItems = this.store.db.prepare(
        "SELECT * FROM work_items WHERE task_group_id = ? ORDER BY created_at, id",
      ).all(group.id).filter((row) => revisionIds.has(row.requirement_revision_id));
      this.#setGroupStatus(group.id, "executing", "confirmation_retry_completed");
      return {
        eventKey,
        taskGroupId: group.id,
        status: "executing",
        classification,
        requirements: existingRevisions.map((row) => ({
          requirementId: row.requirement_id,
          requirementRevisionId: row.id,
          revision: row.revision,
        })),
        affectedWorkItems: [],
        workItems: existingWorkItems.map((item) => item.id),
        executionOrder: existingWorkItems.map((item) => item.id),
      };
    }
    ...
    let plan;
    try {
      plan = await this.#planDemand({
        group,
        content,
        classification,
        suppliedProposal: proposal,
      });
    } catch (error) {
      this.#setGroupStatus(group.id, "collecting", "planning_failed");
      throw error;
    }
    const event = this.store.db.prepare(
      "SELECT * FROM demand_events WHERE event_key = ?",
    ).get(eventKey);
    const requirementResults = [];
    const affectedWorkItems = [];
    const workItems = [];
    for (const requirementPlan of plan.requirements) {
      let requirement = requirementPlan.requirementId
        ? this.store.get("requirements", requirementPlan.requirementId)
        : null;
      if (requirement && requirement.task_group_id !== group.id) {
        throw new Error(`requirement belongs to another task group: ${requirement.id}`);
      }
      if (!requirement && requirementPlan.requirementId) {
        throw new Error(`requirement not found: ${requirementPlan.requirementId}`);
      }
      if (!requirement && classification.type !== "new_requirement" && plan.requirements.length === 1) {
        requirement = this.store.db.prepare(`
          SELECT * FROM requirements WHERE task_group_id = ? AND status <> 'canceled'
          ORDER BY created_at DESC, id DESC LIMIT 1
        `).get(group.id);
      }
      if (!requirement) {
        requirement = this.store.createRequirement({
          taskGroupId: group.id,
      ...
      const revision = this.store.addRequirementRevision({
        ...
        source,
        confirmedAt: event?.confirmed_at || new Date(this.now()).toISOString(),
        normalizedRequirement: requirementPlan.normalizedRequirement,
        acceptanceCriteria: requirementPlan.acceptanceCriteria,
        impactSummary: requirementPlan.impactSummary,
      });
      if (oldRevision) {
        affectedWorkItems.push(...await this.#supersedeAffectedWork(
          oldRevision.id,
          new Map(requirementPlan.impactActions.map(({ workItemId, action }) => [workItemId, action])),
        ));
      }
      const requirementWork = this.#createDag(group.id, revision.id, requirementPlan.workItems);
      workItems.push(...requirementWork);
      requirementResults.push({
        key: requirementPlan.key,
        requirementId: requirement.id,
        requirementRevisionId: revision.id,
        revision: revision.revision,
        workItems: requirementWork.map((item) => item.id),
      });
    }
    this.#recordReplan(group.id, classification.type, {
  ...
  #createDag(taskGroupId, requirementRevisionId, plannedItems) {
    const plans = plannedItems.length
      ? plannedItems
      : defaultPlanner({ content: "Implement requirement", classification: {}, currentRequirement: null }).requirements[0].workItems;
    const byKey = new Map();
    const records = [];
  ...
  async #supersedeAffectedWork(revisionId, explicitActions) {
    ...
    for (const row of items) {
      ...
      const action = explicitActions.get(row.id)
  ...
  async #startWorkItem(group, item, claim) {
    const selectedRuntime = this.#runtimeFor(group.runtime_kind);
    ...
    try {
      ...
      session = this.store.db.prepare(`
        SELECT * FROM worker_sessions
        WHERE work_item_id = ? AND role = 'worker' AND status = 'idle'
          AND runtime_kind = ? AND runtime_session_id IS NOT NULL
        ORDER BY created_at, id LIMIT 1
      `).get(item.id, group.runtime_kind);
      session = session
        ? this.store.prepareWorkerSession(session.id, {
            status: "creating",
            workspace: group.workspace,
            branch,
            worktree: worktree.worktree,
          })
        : this.store.createWorkerSession({
            taskGroupId: group.id,
            workItemId: item.id,
            role: "worker",
            status: "creating",
      ...
      const workerOptions = {
        ...
        onEvent: (event, activeWorker) => {
          try {
            if (event?.type === "session.created" && event.session_id && !session.runtime_session_id) {
              session = this.store.attachRuntimeSession(session.id, event.session_id);
              if (group.runtime_kind === "codex") {
                void this.publishThread({
            ...
            this.onWorkerEvent({
              ...
              runId: run?.id || null,
              event,
              inspection: selectedRuntime.inspectWorker?.(activeWorker) || null,
            });
            if (
              event?.type === "run.completed"
              && Number(event?.data?.usage?.input_tokens) >= this.contextRotateAtTokens
            ) {
              activeWorker.contextLimitReached = true;
              selectedRuntime.interruptWorker(activeWorker);
            }
            if (
      ...
      worker = session.runtime_session_id
        ? selectedRuntime.resumeThread(session.runtime_session_id, JSON.stringify(handoff, null, 2), workerOptions)
        : await selectedRuntime.createWorker(JSON.stringify(handoff, null, 2), workerOptions);
      const runtimeSessionId = worker?.sessionId || worker?.threadId || null;
      if (runtimeSessionId && !session.runtime_session_id) {
        session = this.store.attachRuntimeSession(session.id, runtimeSessionId);
        if (group.runtime_kind === "codex") {
          await this.publishThread({
      ...
      this.runtime.set(session.id, {
        ...
        runtimeKind: group.runtime_kind,
        worker,
        runId: run.id,
        lease,
        locks,
      ...
      if (worker) {
        try { selectedRuntime.interruptWorker(worker); } catch {}
      }
  ...
  async failWorker(input) {
    ...
    this.#setWorkStatus(session.work_item_id, "blocked", "worker_process_failed");
    this.#recordReplan(session.task_group_id, "worker_process_failed", {
      workItemId: session.work_item_id,
      evidenceId: evidence.id,
      blocked: true,
    });
    return { result: "failed", status: "blocked", evidenceId: evidence.id };
  }
  ...
  async recoverWorker(input) {
    ...
    const previous = this.store.db.prepare(`
      ...
      WHERE work_item_id = ? AND type = 'model_call' AND content_hash = ?
    `).get(session.work_item_id, failureFingerprint);
    const attempt = Number(previous.count) + 1;
    const action = failure.recoveryAction || (attempt === 1 ? "reconnect_runtime" : "rebuild_runtime_session");
    const evidence = this.store.addEvidence({
      taskGroupId: session.task_group_id,
      workItemId: session.work_item_id,
      runId: run.id,
      type: "model_call",
      source: session.runtime_kind,
      contentHash: failureFingerprint,
      metadata: { ...failure, recoveryAttempt: attempt, recoveryAction: action },
    });
    const recoverable = failure.recoverable !== false && attempt < this.failureThreshold;
    await this.#stopWorkItem(session.work_item_id, recoverable ? "model_call_recovery" : "model_call_recovery_exhausted", recoverable);
    this.#recordReplan(session.task_group_id, recoverable ? "model_call_recovery" : "model_call_recovery_exhausted", {
      workItemId: session.work_item_id,
      evidenceId: evidence.id,
    ...
    if (!recoverable) {
      this.#setWorkStatus(session.work_item_id, "blocked", failure.category || "model_call_recovery_exhausted");
      return { status: "blocked", attempt, evidenceId: evidence.id, started: [] };
    }
    if (attempt === 1) {
      this.store.prepareWorkerSession(session.id, {
        status: "idle",
    ...
    const delayMs = failure.category === "upstream_rate_limited"
      ? Math.max(1_000, Number(failure.retryAfterMs) || attempt * 5_000)
      : ["upstream_transient_failure", "upstream_request_timeout"].includes(failure.category)
        ? attempt * 1_000
        : 0;
    if (delayMs) await this.wait(delayMs);
    const started = await this.schedule(session.task_group_id);
    return { status: "recovering", attempt, evidenceId: evidence.id, started };
  }
  ...
  async #acceptWorkItem(workItemId, reviewerRun, evidence, workerSession) {
    ...
    try {
      ...
      this.#recordReplan(item.task_group_id, "integration_failed", {
        workItemId: item.id,
        blocked,
        error: error.message,
      });
    ...
    this.#recordReplan(item.task_group_id, "work_item_accepted", {
      workItemId: item.id,
      requirementRevisionId: item.requirement_revision_id,
    });
  ...
  async #rejectWorkItem(workItemId, reviewerRun, evidence, verification, workerSession) {
    ...
    if (Number(repeated) >= this.failureThreshold) {
      ...
      this.#recordReplan(item.task_group_id, "verification_failed", {
        workItemId: item.id,
        failureFingerprint,
        attempts: Number(repeated),
        blocked: true,
      });
  ...
  async resume(taskGroupId) {
    ...
    this.#recordReplan(taskGroupId, "user_resume");
  ...
  async stopWorkItem(taskGroupId, workItemId) {
    ...
    if (!item || item.task_group_id !== taskGroupId) {
      throw new Error(`WorkItem not found in TaskGroup: ${workItemId}`);
    }
    if (!["assigned", "running"].includes(item.status)) {
      throw new Error(`WorkItem ${workItemId} is not running`);
    }
    await this.#stopWorkItem(item.id, "user_stopped_work_item", false);
    this.#setWorkStatus(item.id, "blocked", "user_stopped_work_item");
    this.#recordReplan(taskGroupId, "user_stopped_work_item", { workItemId: item.id });
    return { workItemId: item.id, status: "blocked" };
  }
  ...
  async retryWorkItem(taskGroupId, workItemId) {
    ...
    const group = this.store.get("task_groups", taskGroupId);
    const item = this.store.get("work_items", requireText(workItemId, "workItemId"));
    if (!group || !item || item.task_group_id !== taskGroupId) {
      throw new Error(`WorkItem not found in TaskGroup: ${workItemId}`);
    }
    if (group.status === "paused") throw new Error("paused task group must be resumed before retry");
    if (group.status === "canceled") throw new Error("canceled task group cannot retry work");
    if (!["blocked", "failed"].includes(item.status)) {
      throw new Error(`WorkItem ${workItemId} is not waiting for retry`);
    }
    const activeRun = this.store.db.prepare(`
      SELECT 1 FROM runs WHERE work_item_id = ? AND status = 'running' LIMIT 1
    `).get(item.id);
    if (activeRun) throw new Error(`WorkItem ${workItemId} still has a running Run`);
    this.#setWorkStatus(item.id, "ready", "user_retry_work_item");
    if (group.status === "blocked") this.#setGroupStatus(group.id, "executing", "user_retry_work_item");
    this.#recordReplan(taskGroupId, "user_retry_work_item", { workItemId: item.id });
    return { workItemId: item.id, status: "ready" };
  }
  ...
  async recover(options = {}) {
    ...
    for (const { id } of staleSessions) {
      ...
      if (runtime?.worker) {
        try { runtime.runtime.interruptWorker(runtime.worker); } catch {}
      }
    }
    ...
    for (const taskGroupId of taskGroups) {
      this.#recordReplan(taskGroupId, "worker_recovered", {
        workItems: recovered.readyWorkItems.filter(
          (workItemId) => this.store.get("work_items", workItemId)?.task_group_id === taskGroupId,
        ),
      });
      started.push(...await this.schedule(taskGroupId));
    }
    return { ...recovered, resumedAcceptances, started, recoveredWithinMs: 0 };
  }

  #recordReplan(taskGroupId, reason, payload = {}) {
    this.store.recordExternalEvent({
      taskGroupId,
      aggregateType: "task_group",
      aggregateId: taskGroupId,
      eventType: "project_manager.replanned",
  ...
  async #stopWorkersForDeletion(sessions, workItemIds) {
    for (const { id } of sessions) {
      ...
      if (runtime?.worker) {
        try { runtime.runtime.interruptWorker(runtime.worker); } catch {}
        await runtime.runtime.closeWorker(runtime.worker);
      }
    }
  ...
  async #closeWorkerSession(sessionId, reason) {
    ...
    const runtime = this.runtime.get(sessionId);
    if (runtime?.worker) {
      if (session.role === "worker" && session.work_item_id) runtime.runtime.forgetWorker(runtime.worker);
      else try { await runtime.runtime.closeWorker(runtime.worker); } catch {}
    }
