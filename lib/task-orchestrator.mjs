...
function defaultClassifier({ content, hasRequirements }) {
  ...
  else if (/^(approve|approved|确认|同意|可以执行|执行吧)$/i.test(exact)) ({ type, confidence } = { type: "approval", confidence: 0.99 });
  ...
  return { type, confidence: highImpact ? Math.min(confidence, 0.6) : confidence, highImpact };
}
...
export class TaskOrchestrator {
  constructor(options = {}) {
    ...
    this.maxConcurrency = Math.min(3, Math.max(1, options.maxConcurrency ?? 3));
  ...
  async #ingestDemand(input) {
    ...
    let result;
    const requiresConfirmation = classification.type !== "approval" && (
      !hasRequirements && classification.type === "new_requirement"
    ...
    if (requiresConfirmation) {
      ...
      const pending = this.store.db.prepare(`
        ...
        WHERE task_group_id = ? AND result_json LIKE '%"status":"awaiting_confirmation"%'
  ...
  async verifyReported(workItemId, options = {}) {
    ...
    let verification;
    try {
      this.workspaceManager.assertWriteScope({ worktree: workerSession?.worktree, writeSet: item.write_set });
  ...
  async #acceptWorkItem(workItemId, reviewerRun, evidence, workerSession) {
    ...
    if (workerSession?.worktree && typeof this.workspaceManager.commitWorktree === "function") {
      const commit = await this.workspaceManager.commitWorktree({
    ...
    try {
      ...
      const current = this.store.get("work_items", item.id);
      this.#setWorkStatus(item.id, "rework", "integration_failed");
      this.#setWorkStatus(item.id, "ready", "integration_failed");
      this.#setGroupStatus(item.task_group_id, "executing", "integration_failed");
      return {
        result: "failed",
        phase: "integration",
        workItemId: item.id,
  ...
  async recover(options = {}) {
    ...
    return { ...recovered, started, recoveredWithinMs: 0 };
  }
  ...
  #activeWorkerCount() {
    return Number(this.store.db.prepare(`
      ...
      WHERE role = 'worker' AND status IN ('creating','running','waiting','closing')
    `).get().count);
  }
