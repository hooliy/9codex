...
export class TaskOrchestrator {
  ...
  async #processDemand({ group, sourceMessageId, content, classification, eventKey }) {
    ...
    const existingRevision = this.store.db.prepare(`
      SELECT rr.* FROM requirement_revisions rr
      JOIN requirements requirement ON requirement.id = rr.requirement_id
      WHERE requirement.task_group_id = ? AND rr.source_message_id = ?
      ORDER BY rr.revision DESC LIMIT 1
    `).get(group.id, sourceMessageId);
    if (existingRevision) {
      const existingWorkItems = this.store.db.prepare(`
        SELECT * FROM work_items WHERE requirement_revision_id = ? ORDER BY created_at, id
      `).all(existingRevision.id);
      await this.#provisionWorkItemThreads(group, existingWorkItems);
      ...
      return {
        ...
        status: "executing",
        classification,
        requirementId: existingRevision.requirement_id,
        requirementRevisionId: existingRevision.id,
    ...
    await this.#provisionWorkItemThreads(group, workItems);
  ...
  async #provisionWorkItemThreads(group, workItems) {
    if (typeof this.adapter.createThread !== "function") return;
    await Promise.all(workItems.map(async (item) => {
      const existing = this.store.db.prepare(`
        SELECT * FROM worker_sessions
        WHERE work_item_id = ? AND role = 'worker' AND codex_thread_id IS NOT NULL
        ORDER BY created_at, id LIMIT 1
      `).get(item.id);
      if (existing) {
        try {
          await this.publishThread({ threadId: existing.codex_thread_id, title: item.title, workspace: group.workspace });
          return;
        } catch {}
      }
      const handoff = this.#handoff(group, item, { id: "pending" }, group.workspace);
      const worker = await this.adapter.createThread(JSON.stringify(handoff, null, 2), {
        ...this.workerOptions,
        cwd: group.workspace,
      });
      const threadId = worker?.threadId || worker?.sessionId;
      if (!threadId) throw new Error(`Codex thread was not created for WorkItem ${item.id}`);
      ...
      await this.publishThread({ threadId, title: item.title, workspace: group.workspace });
    }));
  }

  async provisionLatestWorkItemThreads() {
    const group = this.store.db.prepare(`
      SELECT task_group.* FROM task_groups task_group
      WHERE EXISTS (
        SELECT 1 FROM work_items work
        WHERE work.task_group_id = task_group.id
          AND work.status NOT IN ('closed','passed','canceled','stale')
      )
      ORDER BY task_group.updated_at DESC, task_group.id DESC LIMIT 1
    `).get();
    if (!group) return [];
    const items = this.store.db.prepare(`
      SELECT * FROM work_items
      WHERE task_group_id = ? AND requirement_revision_id = ?
        AND status NOT IN ('closed','passed','canceled','stale')
      ORDER BY created_at, id
    `).all(group.id, group.current_requirement_revision_id);
    await this.#provisionWorkItemThreads(group, items);
    ...
    try {
      ...
      const workerOptions = {
        ...
        onEvent: (event, activeWorker) => {
          try {
            if (event?.type === "thread.started" && activeWorker.threadId && !session.codex_thread_id) {
              session = this.store.attachWorkerConversation(session.id, activeWorker.threadId);
              void this.publishThread({
                threadId: activeWorker.threadId,
                title: item.title,
                workspace: group.workspace,
              });
            }
      ...
      worker = session.codex_thread_id
        ? this.adapter.resumeThread(session.codex_thread_id, JSON.stringify(handoff, null, 2), workerOptions)
        : await this.adapter.createWorker(JSON.stringify(handoff, null, 2), workerOptions);
      const codexThreadId = worker?.threadId || worker?.sessionId || null;
      if (codexThreadId && !session.codex_thread_id) {
  ...
  async #closeWorkerSession(sessionId, reason) {
    ...
    const runtime = this.runtime.get(sessionId);
    if (runtime?.worker) {
      if (session.role === "worker" && session.work_item_id) this.adapter.forgetWorker?.(runtime.worker);
      else try { await this.adapter.closeWorker(runtime.worker); } catch {}
    }
