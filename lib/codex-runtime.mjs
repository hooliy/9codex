...
export class CodexRuntime {
  ...
  inspectWorker(workerOrId) {
    const inspection = this.adapter.inspectWorker(workerOrId);
    const event = inspection.lastEvent;
    const failed = ["error", "turn.failed", "adapter.process_error", "adapter.jsonl_error"].includes(event?.type);
    const retrying = Number(inspection.retrySignalCount || 0) >= 3;
    const rawFailure = failed || retrying ? {
      code: event?.code || event?.error?.code,
      message: event?.message || event?.text || event?.error?.message || event?.type,
    } : null;
    return {
      ...inspection,
