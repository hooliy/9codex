...
export function buildMacDesktopLaunch(options = {}) {
  ...
  const script = [
    ...
    `  if [ "$(/usr/bin/osascript -e 'application id "com.openai.codex" is running')" = "true" ]; then`,
    `    /usr/bin/osascript -e 'tell application id "com.openai.codex" to quit'`,
    "    /bin/sleep 1",
    "    if [ \"$({ /usr/bin/osascript -e 'application id \"com.openai.codex\" is running'; } 2>/dev/null)\" = \"true\" ]; then /usr/bin/pkill -TERM -f '^/Applications/ChatGPT[.]app/Contents/MacOS/ChatGPT( |$)' || true; fi",
    "    /bin/sleep 1",
    "    if [ \"$({ /usr/bin/osascript -e 'application id \"com.openai.codex\" is running'; } 2>/dev/null)\" = \"true\" ]; then /usr/bin/pkill -KILL -f '^/Applications/ChatGPT[.]app/Contents/MacOS/ChatGPT( |$)' || true; fi",
    "    deadline=$(( $(/bin/date +%s) + 2 ))",
    `    while [ "$(/usr/bin/osascript -e 'application id "com.openai.codex" is running')" = "true" ]; do`,
