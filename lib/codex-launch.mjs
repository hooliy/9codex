...
export function buildCodexAppServerArguments(options = {}) {
  ...
  return [
    ...
    ...override("model_auto_compact_token_limit_scope", "body_after_prefix"),
...
export function buildMacDesktopLaunch(options = {}) {
  const wrapperPath = requireString(options.wrapperPath, "wrapperPath");
  const openCommand = options.openWorkspace === false
    ? "/usr/bin/open -b com.openai.codex"
    : `/usr/bin/open ${shellLiteral(desktopUri(requireString(
