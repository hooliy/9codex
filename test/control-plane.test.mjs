...
import { saveConfigAtomic } from "../lib/config.mjs";
...
function baseConfig(baseUrl) {
  return {
    ...
    models: { namespace: "9codex", refresh_interval_seconds: 300, available: [] },
...
test("refreshes an access token, retries bootstrap, and atomically applies a valid revision", async (t) => {
  ...
  saveConfigAtomic(paths, baseConfig(fixture.baseUrl));

  const result = await syncBootstrap(paths, baseConfig(fixture.baseUrl));
